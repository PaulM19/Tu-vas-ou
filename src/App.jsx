...
export default function App() {
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", padding: "12px 14px", background: T.bgHover, borderRadius: T.radius, border: `1px solid ${T.border}` }}>
                  <input
                    ...
                    style={{ marginTop: 2, width: 16, height: 16, flexShrink: 0, cursor: "pointer", accentColor: T.accent }}
                  />
                  <span style={{ fontSize: 13, color: T.muted, lineHeight: 1.5 }}>
                    ...
                    <span
                      ...
                      style={{ color: T.text, textDecoration: "underline", cursor: "pointer" }}
                    >
  ...
  if (screen === "privacy") return (
    <div style={{ minHeight: "100vh", background: T.bg, padding: "2rem 1rem" }}>
      <div style={{ maxWidth: 520, margin: "0 auto" }}>
        <button onClick={() => setScreen(registered ? "matches" : "home")} style={{ background: "none", border: "none", cursor: "pointer", color: T.muted, fontSize: 13, padding: "0 0 32px", display: "flex", alignItems: "center", gap: 4 }}>
          ← Retour
        </button>
        ...
        {[
          {
            ...
            content: "Tu pars où ? est une application étudiante créée par des étudiants emlyon pour les étudiants emlyon partant en échange académique. Ce projet n'est pas affilié officiellement à emlyon business school."
          },
          {
            title: "Quelles données collectons-nous ?",
            content: "Lors de ton inscription, nous collectons : ton prénom et nom, ton adresse email @edu.em-lyon.com, ta destination d'échange (école, ville, pays), ton semestre, et optionnellement ton numéro WhatsApp."
          },
          {
            title: "Pourquoi collectons-nous ces données ?",
            content: "Uniquement pour te mettre en relation avec d'autres étudiants emlyon partant à la même destination. Aucune donnée n'est utilisée à des fins commerciales ou transmise à des tiers."
          },
          {
            title: "Qui peut voir mes données ?",
            content: "Seuls les étudiants emlyon inscrits sur la plateforme partant à la même destination que toi peuvent voir ton nom et ton numéro WhatsApp. Ton email n'est jamais affiché publiquement."
          },
          {
            title: "Combien de temps garde-t-on tes données ?",
            content: "Tes données sont conservées jusqu'à la fin de l'année académique 2026-2027, ou jusqu'à ce que tu demandes leur suppression."
          },
          {
            title: "Tes droits (RGPD)",
            content: "Tu as le droit d'accéder à tes données, de les corriger, et de les supprimer à tout moment via le bouton 'Supprimer mon inscription' sur ta page de matchs. Pour toute question, contacte : [adresse à compléter]"
          },
          {
            title: "Sécurité",
            content: "Les données sont stockées sur Supabase (infrastructure sécurisée basée en Europe). Ton numéro WhatsApp n'est visible que par les étudiants qui partagent ta destination."
          },
        ].map(({ title, content }) => (
        ...
        <div style={{ padding: "14px 16px", background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: T.radius, marginTop: 12 }}>
          <p style={{ margin: 0, fontSize: 13, color: T.muted }}>
            Contact RGPD : <strong>[adresse à compléter]</strong>
          </p>
        </div>
      </div>
    </div>
